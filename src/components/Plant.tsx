...
export default function Plant(props: Props) {
  ...
  function createCSS() {
    ...
    const obj: JSX.CSSProperties = {
      "--off-set": getOffSet(props.count, maxClicks, 250),
      "--stem-width": getStemWidth(props.count, maxClicks),
    };

    for (const leaf in leafPoints) {
      const points = leafPoints[leaf];
      obj[`--${leaf}-scale`] = Math.min(props.count / (points * 2), 1);
    }
  ...
  const interval = setInterval(() => {
    setCount(props.count + maxClicks / 300);
  }, 10);

  createEffect(() => {
    if (props.count >= maxClicks) {
